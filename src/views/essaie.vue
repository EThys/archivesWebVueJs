...
const file = ref<Array<IDataFile>>([])
// const fileName = computed(() => file.value);
...
const submitInvoice = async () => {
  ...
  await useAxiosRequestWithToken(token)
    .post(`${ApiRoutes.addInvoice}`, data, { signal: abortSignal })
...
const InvoiceInputValidate = async () => {
  loading.value = true
  if (
    !invoice.value.InvoiceCode ||
    !invoice.value.InvoiceBarCode ||
    invoice.value.InvoiceCode.trim() === '' ||
    invoice.value.InvoiceBarCode.trim() === ''
  ) {
    setTimeout(() => {
      toast.open({
